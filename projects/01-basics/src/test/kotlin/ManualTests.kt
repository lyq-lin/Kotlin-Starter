...
import basics.ConsoleSender
import basics.Environment
import basics.NotificationService
...
fun main() {
    ...
    assertEquals(5, safeDivide(10, 2).getOrNull(), "resultSuccess")
    assertTrue(safeDivide(10, 0).isFailure, "resultFailure")
    assertEquals(1002, firstLargeEven((1..600).asSequence()), "sequenceLazy")

    val notifier = NotificationService(ConsoleSender())
    val token1 = notifier.token
    val token2 = notifier.token
    assertEquals(token1, token2, "lazyTokenOnce")
